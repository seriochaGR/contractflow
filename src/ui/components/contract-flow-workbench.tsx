...
export function ContractFlowWorkbench() {
  ...
  function openDownloadDialog() {
    if (!canDownloadArchive) return;
    setArchiveName(buildDefaultArchiveName(rootModelName));
    setShowDownloadDialog(true);
  }

  async function downloadArchive() {
    if (!output) return;
    const normalizedName = buildDefaultArchiveName(archiveName);
    ...
    try {
      const response = await fetch("/api/export/archive", {
        ...
        body: JSON.stringify({
          ...
          files: {
            contracts: output.typescript,
            angularService: output.angularService,
      ...
      if (!response.ok) {
        ...
        throw new Error(payload?.error ?? "Archive export failed.");
      }
      ...
      const message = err instanceof Error ? err.message : "Archive export failed.";
      setNotification({ kind: "error", message });
    } finally {
    ...
    <>
      <main className="mx-auto flex h-[calc(100vh-4rem)] max-w-[1600px] flex-col overflow-hidden px-4 py-4 md:px-6">
        ...
        <section className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[minmax(0,0.94fr)_minmax(0,1.06fr)_3.5rem]">
          ...
          <article className="flex min-h-0 flex-col rounded-2xl border border-slate-800 bg-panel/90 p-3 backdrop-blur-xl">
            <div className="mb-3 flex min-h-11 min-w-0 items-center justify-between gap-2">
      ...
      {showDownloadDialog ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-panel/95 p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-lg font-semibold text-slate-100">Download Project Zip</h2>
                <p className="mt-1 text-sm text-slate-400">Choose the root folder name for the exported Angular scaffolding.</p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-5 space-y-3">
              <label className="block text-sm text-slate-300">
                <span className="mb-2 block text-xs uppercase tracking-wide text-slate-500">Root Folder Name</span>
                <input
                  value={archiveName}
                  ...
                  placeholder="contractflow-export"
                  autoFocus
                />
              </label>
              <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs text-slate-400">
                <div>{buildDefaultArchiveName(archiveName)}/</div>
                <div className="mt-1">components/ models/ services/ mocks/</div>
              </div>
            </div>
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button
                ...
                className={`rounded-md px-3 py-2 text-sm ${isDownloading
                  ? "cursor-wait border border-cyan-300/30 bg-cyan-400/10 text-cyan-100"
                  : "border border-cyan-300/40 bg-cyan-400/15 text-cyan-100 hover:bg-cyan-400/20"
                }`}
              >
...
function OutputSidebarRail({
  ...
  return (
    <div className="flex h-14 w-full shrink-0 flex-row gap-2 border-t border-slate-800 bg-slate-950/95 p-2 shadow-[0_-12px_24px_rgba(15,23,42,0.18)] lg:h-full lg:w-14 lg:flex-col lg:border-l lg:border-t-0 lg:shadow-[-12px_0_24px_rgba(15,23,42,0.28)]">
      <div className="flex flex-1 flex-row gap-1 lg:flex-col">
        {outputTabs.map((tab) => (
          <SidebarRailButton
            key={tab.key}
...
function SidebarRailButton({
  ...
  return (
    <div className={`group/rail relative ${alignBottom ? "lg:mt-auto" : ""}`}>
      <button
        type="button"
        onClick={onClick}
        aria-label={label}
      ...
      <div className="pointer-events-none absolute bottom-full left-1/2 z-30 mb-3 -translate-x-1/2 opacity-0 transition duration-150 group-hover/rail:opacity-100 group-focus-within/rail:opacity-100 lg:bottom-auto lg:left-auto lg:right-full lg:top-1/2 lg:mb-0 lg:mr-3 lg:translate-x-0 lg:-translate-y-1/2">
        <div className="relative whitespace-nowrap rounded-md border border-slate-700 bg-slate-950 px-2.5 py-1.5 text-xs font-medium text-slate-100 shadow-lg">
          {label}
          <span className="absolute left-1/2 top-full h-2 w-2 -translate-x-1/2 -translate-y-1/2 rotate-45 border-b border-r border-slate-700 bg-slate-950 lg:left-full lg:top-1/2 lg:-translate-x-1/2 lg:-translate-y-1/2 lg:border-b-0 lg:border-r lg:border-t" />
        </div>
      </div>
    </div>
  );
...
function buildDefaultArchiveName(value: string): string {
